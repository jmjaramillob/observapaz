...
class SoloLecturaPublicaPermiso(permissions.BasePermission):
    ...
    información no sensible (observatorios, categorías, indicadores).
...
class RegistroIndicadorPermiso(permissions.BasePermission):
    """
    - 'create' (el formulario público envía aquí su registro) y 'resumen'
      (cifras agregadas para el tablero público): abiertos a cualquiera.
    - Todo lo demás -listar o ver el detalle de los registros, que
      incluye fuente/observaciones/ubicación- requiere sesión iniciada.
      El alcance por observatorio se aplica en get_queryset() del
      ViewSet, no aquí.
    """
